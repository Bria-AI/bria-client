...
class VideoGenerationAPI(StatusBasedAPI):
    ...
    async def from_tailored_image(self, payload: GenerateByTailoredImageRequestPayload) -> Response:
        """
        Generate a video by a tailored image

        Args:
            `payload: GenerateByTailoredImageRequestPayload` - The payload for the video generation request
