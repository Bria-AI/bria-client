...
from bria_client.schemas.image_editing_apis.product_integrate import ProductIntegrateRequestPayload


class ProductIntegrateAPI(StatusBasedAPI):
    ...
    async def product_integrate(self, payload: ProductIntegrateRequestPayload) -> Response:
        """
        Integrate and embed one or more products into a scene at user-defined coordinates.

        Args:
            `payload: ProductIntegrateRequestPayload` - The payload for the product integrate request
        ...
        try:
            response: Response = await self._engine_client.post(
                BriaEngineAPIRoutes.V2_IMAGE_EDIT_PRODUCT_INTEGRATE,
